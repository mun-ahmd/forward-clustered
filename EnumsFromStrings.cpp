...
static std::unordered_map<std::string, VkImageLayout> layoutMap = {
...
static std::unordered_map<std::string, VkPolygonMode> polygonModeMap = {
...
static std::unordered_map<std::string, VkCompareOp> compareOpMap = {
...
static std::unordered_map<std::string, VkShaderStageFlags> shaderStageMap = {
	...
	{"flagMaxEnum", VK_SHADER_STAGE_FLAG_BITS_MAX_ENUM }
};

VkImageLayout Rendering::layoutFromString(std::string layout) {
	//use map
	auto value = layoutMap.find(layout);
	if (value != layoutMap.end()) {
		//mapping found
		return value->second;
	}
	else {
		std::cerr << "Could not find matching VkImageLayout for the format: " << layout << std::endl;
		assert(false);
		return VK_IMAGE_LAYOUT_MAX_ENUM;
	}
}

VkFormat Rendering::formatFromString(std::string format) {
	auto value = formatMap.find(format);
	if (value != formatMap.end()) {
		//mapping found
		return value->second;
	}
	else {
		std::cerr << "Could not find matching VkFormat for the format: " << format << std::endl;
		assert(false);
		return VK_FORMAT_MAX_ENUM;
	}
}

VkPolygonMode Rendering::polygonModeFromString(std::string mode) {
	auto value = polygonModeMap.find(mode);
	if (value != polygonModeMap.end()) {
		//mapping found
		return value->second;
	}
	else {
		std::cerr << "Could not find matching VkPolygonMode for the mode: " << mode << std::endl;
		assert(false);
		return VK_POLYGON_MODE_MAX_ENUM;
	}
}

VkCompareOp Rendering::compareOpFromString(std::string op) {
	auto value = compareOpMap.find(op);
	if (value != compareOpMap.end()) {
		//mapping found
		return value->second;
	}
	else {
		std::cerr << "Could not find matching VkCompareOp for the op: " << op << std::endl;
		assert(false);
		return VK_COMPARE_OP_MAX_ENUM;
	}
}

VkShaderStageFlags stageFlagFromMap(std::string stage) {
	auto value = shaderStageMap.find(stage);
	if (value != shaderStageMap.end()) {
		//mapping found
		return value->second;
	}
	else {
		std::cerr << "Could not find matching VkShaderStage for the stage: " << stage << std::endl;
		assert(false);
		return VK_FORMAT_MAX_ENUM;
	}
}

VkShaderStageFlags Rendering::shaderStageFlagsFromString(std::string shaderStages) {
	uint32_t substrStart = 0;
	VkShaderStageFlags flags = 0;
	for (uint32_t i = 0; i < shaderStages.size(); i++) {
		if (shaderStages[i] == '_') {
			flags |= stageFlagFromMap(shaderStages.substr(substrStart, i - substrStart));
			substrStart = i + 1;
		}
	}
	return flags;
