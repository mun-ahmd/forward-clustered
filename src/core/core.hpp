...
private:
	...
	static int physicalDeviceTypePreference(VkPhysicalDeviceType t) {
		// Prefer discrete > virtual (e.g. VM passthrough) > integrated > CPU > other.
		switch (t) {
	...
	void pickPhysicalDevice() {
		...
		uint64_t bestDeviceLocalHeap = 0;
		...
		for (const auto& device : devices) {
			...
			const int preference = physicalDeviceTypePreference(props.deviceType);
			...
			for (uint32_t i = 0; i < memProps.memoryHeapCount; ++i) {
				if (memProps.memoryHeaps[i].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) {
					deviceLocalHeap = std::max(deviceLocalHeap, memProps.memoryHeaps[i].size);
				}
			}

			if (preference > bestPreference
				|| (preference == bestPreference && deviceLocalHeap > bestDeviceLocalHeap)) {
				bestDevice = device;
				bestPreference = preference;
				bestDeviceLocalHeap = deviceLocalHeap;
			}
		}
