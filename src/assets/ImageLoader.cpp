...
glm::ivec3 getImageInfo(const char* filepath) {
	...
	if (!stbi_info(filepath, &info.x, &info.y, &info.z)) {
		throw std::runtime_error(std::string("getImageInfo: could not read image header (missing or unsupported file): ") + filepath);
	}
	if (info.x <= 0 || info.y <= 0) {
		throw std::runtime_error(std::string("getImageInfo: invalid image dimensions for: ") + filepath);
	}
	return info;
}
