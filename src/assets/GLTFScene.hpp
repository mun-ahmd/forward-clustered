...
private:
	...
	void loadMaterials(std::vector<MaterialPBR>&& materialsIN) {
		materials.addMaterialImage(
			createSolidColorTexture2D(VulkanUtils::utils().getCore(), std::array<uint8_t, 4>{ 255, 255, 255, 255 }, true),
			vSampler
		);
...
public:
	...
	void clearScene() {
		// Mesh destructors free GPU buffers; they must not run while draws referencing
		// those buffers may still be in flight on the queue.
		VulkanCore core = VulkanUtils::utils().getCore();
		vkDeviceWaitIdle(core->device);
		// Drain completed uploads before destroying materials/images (see processResults).
