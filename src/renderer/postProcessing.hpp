...
VkDescriptorSet createPostProcessDS(VulkanCore core) {
	...
	binding2 = binding0;
	binding2.binding = 2;
	return core->createDescriptorSet({ binding0, binding1, binding2 });
}
...
private:
	...
	void initPipeline(VkPipelineLayout layout) {
		...
		auto vertShaderCode = VulkanUtils::utils().compileGlslToSpv("Shaders/postProcess.vert", shaderc_shader_kind::shaderc_vertex_shader);
		auto fragShaderCode = VulkanUtils::utils().compileGlslToSpv("Shaders/postProcess.frag", shaderc_shader_kind::shaderc_fragment_shader);
		...
		renderingCreateInfo.pColorAttachmentFormats = &this->colorOutputImage->format;
		...
		// Dynamic rendering: subpass must be 0 when renderPass is VK_NULL_HANDLE.
...
public:
	...
	static std::shared_ptr<PostProcessRenderer> create(
		VulkanCore core,
		VkRect2D renderRegion,
		VkPipelineLayout layout
	) {
		return std::make_shared<PostProcessRenderer>(
			core, renderRegion, layout
		);
	}
	...
	void resize(VkRect2D newRegion) {
		colorOutputImage.reset();
		colorOutputView.reset();
		initRenderAreas(newRegion);
		initResources();
	}
