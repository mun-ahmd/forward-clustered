...
#include "renderer/depthOnlyRenderer.hpp"
...
public:
	...
	void resize(VkRect2D newRegion) {
		...
		forward->resize(newRegion);
		postProcess->resize(newRegion);
		setTexturesToPostProcessDS(
			forward->colorOutputImage,
			forward->depthImage,
			forward->gNormal
	...
	VkDescriptorSet getGlobalDescriptorSet(uint32_t frameIndex) const {
		assert(frameIndex < FRAMES_IN_FLIGHT);
		return perFrame[frameIndex].globalFrameData.descriptor;
	}

	VkDescriptorSet getPointLightsDescriptorSet(uint32_t frameIndex) const {
		assert(frameIndex < FRAMES_IN_FLIGHT);
		return perFrame[frameIndex].pointLightsDS;
	}

	/** CPU destination for per-frame global UBO (same memory the global descriptor set uses). */
	void* getGlobalDescriptorMappedData(uint32_t frameIndex) const {
		assert(frameIndex < FRAMES_IN_FLIGHT);
		return perFrame[frameIndex].globalFrameData.mappedBuffer->getMappedData();
	}
};
