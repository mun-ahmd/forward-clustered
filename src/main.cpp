...
struct FrameData {
	void* globalDescBufferMappedPointer = nullptr;
...
public:
	...
	~Application() {
		...
		if (clusterComp.pipe != VK_NULL_HANDLE) {
			vkDestroyPipeline(core->device, clusterComp.pipe, nullptr);
		}
		if (clusterComp.layout != VK_NULL_HANDLE) {
...
private:
	...
	void initVulkan() {
		...
		for (uint32_t fi = 0; fi < MAX_FRAMES_IN_FLIGHT; ++fi) {
			frames[fi].data.globalDS = renderer.getGlobalDescriptorSet(fi);
